...
/**
 * 最新回复 详情 教师端 创建人：李林中 创建日期：2014-12-15 上午10:09:44 作用： 修改
 * =================================================== 修改人 修改日期 原因(描述)
 * ===================================================
 */
	...
	private void initData() {
		...
		ll_list_common_ssh_fx.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				String contentString = newCommentsDetail.getDynamicTeacher()
						.getDynamicContent();
				String imgUrlString = "";
				if (newCommentsDetail.getDynamicTeacher().getPhotoList().size() > 0) {
					imgUrlString = URLs.formatImgURL(newCommentsDetail
							.getDynamicTeacher().getPhotoList().get(0)
							.getThumbPath()
							+ "/"
							+ newCommentsDetail.getDynamicTeacher()
									.getPhotoList().get(0).getThumbSaveName());
				}
