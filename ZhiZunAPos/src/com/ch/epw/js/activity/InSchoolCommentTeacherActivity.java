...
public class InSchoolCommentTeacherActivity extends BaseActivity {

	protected static final String TAG = InSchoolCommentTeacherActivity.class
			.getName();
	private TitleBarView titleBarView;
	Context context;
	Integer dataCount;// 在校点评数量 共用字段
	RemarkList remarkList;
	List<Remark> list;
	ListViewRemarkTeacherAdapter listViewRemarkTeacherAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_zxdp_teacher);
		context = this;
		initView();
		new GetNewCommentsCountTask(context, newReplyMsgBadgeView).execute(
				AppContext.getApp().getToken(), Constant.COMMNETTYPE_ZXDP);
		content();
	}
	...
	private void content() {
		...
		mPullListView.setOnRefreshListener(new OnRefreshListener<ListView>() {
			...
			public void onPullDownToRefresh(
				...
				new GetRemarkListTask().execute(1 + "", Constant.LOADDATACOUNT
						+ "");
			}
			...
			public void onPullUpToRefresh(
				...
				Log.i(TAG, "page=" + page);
				new GetRemarkListTask().execute(mCurPage + "",
	...
	private void initView() {
		...
		titleBarView.setTitleText(R.string.in_school_comment);
		...
		mPullListView = (PullToRefreshListView) this
				.findViewById(R.id.ll_activity_zxdp_teacher);
		...
		fl_common_zxhf.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.putExtra("type", Constant.COMMNETTYPE_ZXDP);
	...
	/**
	 * 获取点评 创建人：李林中 创建日期：2014-12-9 上午10:42:07 作用： 修改
	 * =================================================== 修改人 修改日期 原因(描述)
	 * ===================================================
	 */
	private class GetRemarkListTask extends AsyncTask<String, Void, RemarkList> {
		AppException e;

		// 执行这个方法 此方法重新开启一个来执行网络接口读数据的
		// 操作，将得到的结果List<Preferential>，传送给onPostExecute方法
		@Override
		protected RemarkList doInBackground(String... params) {

			try {
				remarkList = AppContext.getApp().getRemarkList(params[0],
						params[1]);
		...
		@Override
		protected void onPostExecute(RemarkList result) {
			...
			if (result != null) {
				...
				if (result.getStatus().equals("0")) {
					...
					if (list != null && list.size() > 0) {
						list.addAll(result.getRemarkList());
						listViewRemarkTeacherAdapter.notifyDataSetChanged();
					} else {
						list = result.getRemarkList();
						listViewRemarkTeacherAdapter = new ListViewRemarkTeacherAdapter(
								InSchoolCommentTeacherActivity.this, list);
						// 得到实际的ListView
						mListView.setAdapter(listViewRemarkTeacherAdapter);
					}
