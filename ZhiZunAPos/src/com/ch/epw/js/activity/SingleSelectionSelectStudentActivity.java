 ...
 * ===================================================
 */
public class SingleSelectionSelectStudentActivity extends BaseActivity {
	...
	private void initView() {
		...
		titleBarView.setBarRightOnclickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				SingleSelectionSelectStudentActivity.this.setResult(
						Constant.SEND_NOTICE_SELCET_SUTDENT, intent);
				SingleSelectionSelectStudentActivity.this.finish();
				backAnim();
			}
		});
				...
				.setOnChildClickListener(new OnChildClickListener() {
					...
					public boolean onChildClick(ExpandableListView parent,
						...
						if (studentInfo.getCheckState()) {// 选中了学生
							...
							// 如果本项选择了 那么其他项不要选择
							if (studentClassesList.get(groupPosition)
									.getStudentInfoList().get(childPosition)
									.getCheckState()) {
								for (int i = 0; i < studentClassesList.size(); i++) {
									if (i != groupPosition) {
										studentClassesList.get(i)
												.setCheckState(false);
										for (int j = 0; j < studentClassesList
												.get(i).getStudentInfoList()
												.size(); j++) {
											studentClassesList.get(i)
													.getStudentInfoList()
													.get(j)
	...
	/**
	 * 获取班级和学生 创建人：李林中 创建日期：2014-12-9 上午10:42:07 作用： 修改
	 * =================================================== 修改人 修改日期 原因(描述)
	 * ===================================================
	 */
