...
/**
 * 修改密码第一步 创建人：李林中 创建日期：2014-12-15 上午9:47:50 作用： 修改
 * =================================================== 修改人 修改日期 原因(描述)
 * ===================================================
 */
public class ChangePassword1Activity extends BaseActivity {
	...
	private void initView() {
		...
		tv_forget_passrword_step2_title.setText("验证短信将发送至您的手机：" + phone);
		...
		btn_next_submit.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				new ValidUserTask().execute(Constant.VALIDTYPE_MOBILE,
						Constant.VALIDTYPE_CHANGE_PWD, validText);
			}
		});
		...
		tv_forget_passrword_timer.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				new SendSmsCodeBindTask().execute(phone,
						Constant.VALIDTYPE_AUTHENTICATION);
			}
		});
		// tv_forget_passrword_timer.setEnabled(false);
		// timer.start();
	 ...
	 * ===================================================
	 */
	private class ValidUserTask extends
		...
		protected ValidCodeResult doInBackground(String... params) {
			// TODO Auto-generated method stub
			try {
				validCodeResult = ((AppContext) getApplication()).validdata(
		...
		protected void onPostExecute(ValidCodeResult result) {
			...
			if (null == result) {
				...
				if (result.getStatus().equals("0")) {
					...
					UIHelper.ToastMessage(context, "验证码不正确");
					return;
				}
			}
		}
	...
	public void onResume() {
		super.onResume();
		et_forget_passrword_text.setText("");
		tv_forget_passrword_timer.setText("获取验证码");
		tv_forget_passrword_timer.setEnabled(true);
		timer.cancel();
	}
