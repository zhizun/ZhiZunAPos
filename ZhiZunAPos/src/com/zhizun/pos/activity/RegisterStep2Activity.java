...
/**
 * 注册第2步 创建人：李林中 创建日期：2014-12-15 上午9:47:50 作用： 修改
 * =================================================== 修改人 修改日期 原因(描述)
 * ===================================================
 */
public class RegisterStep2Activity extends BaseActivity {
	...
	Result validSendSmsCodeResult;
	Result validSmsCodeResult;
	String phone;
	String validText;
	...
	private void initView() {
		...
		titleBarView.setTitleText("填写验证码");
		...
		btn_next_submit.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				new ValidSmsCodeTask().execute(phone, Constant.VALIDTYPE_REG,
						validText);
			}
		});
	...
	private class SendCodeTask extends AsyncTask<String, Void, Result> {
		...
		protected Result doInBackground(String... params) {
			// TODO Auto-generated method stub
			try {
				validSendSmsCodeResult = ((AppContext) getApplication())
						.sendSmsCode(params[0], params[1]);
		...
		protected void onPostExecute(Result result) {
			...
			if (null == result) {
				...
				if (result.getStatus().equals("0")) {
					UIHelper.ToastMessage(context, "验证码已发送到" + phone);
				} else {
					timer.cancel();
	 ...
	 * ===================================================
	 */
	private class ValidSmsCodeTask extends AsyncTask<String, Void, Result> {
		AppException e;

		protected Result doInBackground(String... params) {
			// TODO Auto-generated method stub
			try {
				validSmsCodeResult = ((AppContext) getApplication())
						.validateSmsCode(params[0], params[1], params[2]);
			} catch (AppException e) {
				// TODO Auto-generated catch block
				this.e = e;
				e.printStackTrace();
		...
		protected void onPostExecute(Result result) {
			...
			if (null == result) {
				...
				if (result.getStatus().equals("0")) {
					...
					intent.putExtra("smsCode", validText);
