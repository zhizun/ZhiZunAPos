...
public class PhotoShowActivity extends BaseActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		for (int i = 0; i < BaseTools.bimp.getBmp().size(); i++) {
			bmp.add(BaseTools.bimp.getBmp().get(i));
		}
		for (int i = 0; i < BaseTools.bimp.getDrr().size(); i++) {
			drr.add(BaseTools.bimp.getDrr().get(i));
		}
		max = BaseTools.bimp.getMax();
		...
		photo_bt_del.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (listViews.size() == 1) {
					...
					Log.i(TAG, "删除时候的文件名newStr="+newStr);
					bmp.remove(count);
					drr.remove(count);
					del.add(newStr);
					max--;
		...
		photo_bt_enter.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				BaseTools.bimp.setBmp(bmp);
				BaseTools.bimp.setDrr(drr);
				BaseTools.bimp.setMax(max);
