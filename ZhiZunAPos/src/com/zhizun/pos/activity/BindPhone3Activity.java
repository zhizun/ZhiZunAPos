...
public class BindPhone3Activity extends BaseActivity {
	...
	Result result;
	String phone, password, validText;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initView();
		AppManager.getAppManager().finishActivity(BindPhone2Activity.class);
	}
		...
		btn_next_submit.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				new ValidSmsCodeBindTask().execute(phone,
						Constant.VALIDTYPE_BIND, validText);
			}
		});
	...
	private class ValidSmsCodeBindTask extends AsyncTask<String, Void, Result> {
		...
		protected void onPostExecute(Result result) {
			...
			if (null == result) {
				...
				if (result.getStatus().equals("0")) {
					new BindPhoneTask().execute(password, phone, validText);
				} else {
					timer.cancel();
					tv_forget_passrword_timer.setEnabled(true);
					tv_forget_passrword_timer.setText("重新发送");
	...
	/**
	 * 绑定手机 创建人：李林中 创建日期：2015-1-28 下午5:46:11 作用： 修改
	 * =================================================== 修改人 修改日期 原因(描述)
	 * ===================================================
	 */
	private class BindPhoneTask extends AsyncTask<String, Void, Result> {
		AppException e;

		protected Result doInBackground(String... params) {
			// TODO Auto-generated method stub
			try {
				result = ((AppContext) getApplication()).bind(params[0],
						params[1], params[2]);
			} catch (AppException e) {
				// TODO Auto-generated catch block
				this.e = e;
