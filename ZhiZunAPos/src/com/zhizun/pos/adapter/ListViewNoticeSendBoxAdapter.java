...
/**
 * 通知发件箱列表ListViewAdapter
 */
public class ListViewNoticeSendBoxAdapter extends MyBaseAdapter {

	private Context context;// 运行上下文
	private List<NoticeBox> listItems; // 数据集合

	public ListViewNoticeSendBoxAdapter(Context context,
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (noticeBox.getStatus().equals("1")) {
			holder.iv_list_my_notice_item_logo
					.setImageResource(R.drawable.notice_beensent);
		}
		if (noticeBox.getStatus().equals("0")) {
			holder.iv_list_my_notice_item_logo
					.setImageResource(R.drawable.notice_waitsend);
		}
