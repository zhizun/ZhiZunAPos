...
/**
 * 考勤 详细 教师端 ListViewAdapter
 * 
 * @param <index2>
 */
public class ListViewAttenceItemSaveTeacherAdapter extends MyBaseAdapter {

	private static final String TAG = ListViewAttenceItemSaveTeacherAdapter.class
			.getName();
	...
	public ListViewAttenceItemSaveTeacherAdapter(Context context,
			ArrayList<AttenceDetail> listItems, Handler handler, int attencenum,
			int abstentnum) {
		super();
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		holder.iv_list_attence_js_item_listviewitem_yes
				.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						//如果是已到状态，直接返回
						if(listItems.get(position).getStatus().equals(Constant.ATTENCE_STATUS_ATTEND)){
							return;
						}
						if (v.getTag().equals("kqyd_gray")) {
							...
							holder.iv_list_attence_js_item_listviewitem_no
									.setImageResource(R.drawable.kqwd_gray);
							listItems.get(position).setStatus(Constant.ATTENCE_STATUS_ATTEND);
		...
		holder.iv_list_attence_js_item_listviewitem_no
				.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						//如果是未到状态，直接返回
						if(listItems.get(position).getStatus().equals(Constant.ATTENCE_STATUS_ABSENT)){
							return;
						}
						if (v.getTag().equals("kqwd_gray")) {
							...
							holder.iv_list_attence_js_item_listviewitem_no
									.setImageResource(R.drawable.kqwd_red);
							listItems.get(position).setStatus(Constant.ATTENCE_STATUS_ABSENT);
							attencenum = attencenum - 1;
							abstentnum = abstentnum + 1;
							sendMessageHandler();
						}
		...
		holder.iv_list_attence_js_item_listviewitem_bz.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				Intent intent=new Intent(context,AttenceRemarksActivity.class);
